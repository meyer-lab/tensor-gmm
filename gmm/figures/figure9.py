...
def makeFigure():
    ...
    for dose in range(0, 12):
        points = gen_points_GMM(maximizedNK, preNormOptCP, optPTfactors, timei, dose, ligandi, n_samples =cellperexp)
        ...
        sns.scatterplot(data=markertotal.loc[markertotal["Dose"] == dose_unique[dose]],x="pSTAT5",y="CD25",ax=ax[(dose*2)+1], s=5)
